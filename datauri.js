...
DataURI('./libcsound.wasm')
    .then(content =>
          {
              // Old WebAudio
              var file1 = fs.readFileSync('./libcsound.js').toString();
              var replc1 = file1.replace('libcsound.wasm', content);

              // AudioWorklet
              var file2 = fs.readFileSync('./libcsound.js').toString();
  ...
  ENVIRONMENT_IS_SHELL = !ENVIRONMENT_IS_WEB && !ENVIRONMENT_IS_NODE && !ENVIRONMENT_IS_WORKER;`, `ENVIRONMENT_IS_WEB = true;\n`)
             
              replc2 = replc2.replace(`    if (document.currentScript) {
                  scriptDirectory = document.currentScript.src;
              }`, '');
              replc2 = replc2.replace(`var ENVIRONMENT_IS_WEB = false;`,
...
`);
              ...
              try {
                  fs.writeFileSync('./libcsound_browser.js', replc2);
